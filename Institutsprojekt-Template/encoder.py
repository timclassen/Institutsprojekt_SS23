...
def vertical_intra_prediction(block):
    ...
    # Fülle den Block unten mit einer zusätzlichen Reihe auf
    padded_block = np.pad(block, ((0, 1), (0, 0)), mode='edge')

    # Berechne den Gradienten entlang der vertikalen Richtung
    gradient = np.abs(np.diff(padded_block, axis=0))

    # Iteriere über jede Zeile im Block
    for j in range(block_width):
        # Der Vorhersagewert wird aus dem oberen Nachbarpixel abgeleitet
        prediction_values = block[0, j] + np.cumsum(gradient[:, j])

        # Setze die Vorhersagewerte für die aktuelle Zeile im Block
        prediction_block[:, j] = prediction_values
...
def horizontal_intra_prediction(block):
    ...
    # Fülle den Block rechts mit einer zusätzlichen Spalte auf
    padded_block = np.pad(block, ((0, 0), (0, 1)), mode='edge')

    # Berechne den Gradienten entlang der horizontalen Richtung
    gradient = np.abs(np.diff(padded_block, axis=1))

    # Iteriere über jede Zeile im Block
    for i in range(block_height):
        # Der Vorhersagewert wird aus dem linken Nachbarpixel abgeleitet
        prediction_values = block[i, 0] + np.cumsum(gradient[i, :])

        # Setze die Vorhersagewerte für die aktuelle Zeile im Block
        prediction_block[i, :] = prediction_values
...
def diagonal_intra_prediction(block):
    ...
    # Füge dem Block eine zusätzliche Zeile und eine zusätzliche Spalte hinzu
    padded_block = np.pad(block, ((1, 0), (1, 0)), mode='edge')
    ...
    for i in range(block_height):
        ...
        for j in range(block_width):
            ...
            prediction_value = padded_block[i+1, j+1]

            # Setze den Vorhersagewert für das aktuelle Pixel im Vorhersageblock
